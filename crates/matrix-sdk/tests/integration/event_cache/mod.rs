...
async fn test_concurrent_backpagination() {
    ...
    // Sync with a room that has a prev_batch token
    let room = server
        .sync_room(
            &client,
            JoinedRoomBuilder::new(room_id)
    ...
    // Mock a single back-pagination response with a delay to allow for race
    // condition testing
    server
        .mock_room_messages()
        .match_from("prev_batch")
        .ok(RoomMessagesResponseTemplate::default()
            ...
            .with_delay(std::time::Duration::from_millis(200))) // Add delay to ensure concurrency
    ...
    let pagination1 = room_event_cache.pagination();
    let pagination2 = room_event_cache.pagination();

    let (outcome1, outcome2) =
        tokio::join!(pagination1.run_backwards_once(10), pagination2.run_backwards_once(10),);

    // Both should succeed
    let outcome1 = outcome1.unwrap();
    let outcome2 = outcome2.unwrap();

    // One should have the actual events, the other should have empty events (since
    // it joined the first)
    let total_events = outcome1.events.len() + outcome2.events.len();

    // The total should be exactly 2 events (from the single network request)
    // One caller gets the events, the other gets empty (but both succeed)
    assert!(
        total_events == 2 || total_events == 0,
        "Expected 2 total events from concurrent calls, got {}",
        total_events
    );

    // Both should report the same reached_start status
    assert_eq!(outcome1.reached_start, outcome2.reached_start);
    ...
    // Verify the events were actually added to the timeline
    assert_eq!(diffs.len(), 2);
    assert_matches!(&diffs[0], VectorDiff::Insert { index, value: event } => {
        assert_eq!(*index, 0);
    ...
    // Verify that the timeline now has all events in correct order
